...
class TestResetPass:
    ...
    def test_vosstanovit_pass_enter_email_pass(self, driver):
        element = ResetPass(driver).vosstanovit_pass_enter_email()
        assert element == api_urls.RESET_PASS_PAGE_URL
    ...
    def test_check_pass_symbols_visible_pass(self, driver):
        element = ResetPass(driver).check_pass_symbols_visible()
        assert data.PASS_IS_VISIBLE_ELEMENT_IN_HTML in element
